<div class="flex flex-col items-center gap-8">
    @foreach ($posts as $post)
        <article class="flex flex-col md:flex-row gap-6 w-full md:w-[60%] mx-auto overflow-hidden shadow-lg bg-white dark:bg-black border border-gray-700 hover:scale-[1.02] transition duration-300">
            <img src="{{ asset($post->image) }}" class="w-full md:w-64 h-48 md:h-64 object-cover" alt="">
            <div class="p-4">
...
<div class="flex justify-center mt-10 items-center gap-2">
    ...
    @if ($posts->onFirstPage() === false)
        <a href="{{ $posts->previousPageUrl() }}" class="pagination-link px-3 py-1 rounded border bg-white text-gray-800 hover:bg-gray-100">
            ←
        </a>
    @endif
    ...
    @if ($posts->hasMorePages())
        <a href="{{ $posts->nextPageUrl() }}" class="pagination-link px-3 py-1 rounded border bg-white text-gray-800 hover:bg-gray-100">
            →
        </a>
    @endif
</div>
